...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
/* It's because nestedFuction is in the same scope as const internal so it can use it's value.*/
...
function summation(number) {
  const values = [{num: 1}, {num: 2}, {num: 3}, {num: 4}
  ];
  const total = values.reduce((currentTotal, value) => {
    return value.num + currentTotal
  }, 0)
  return total;
  }
...
const zooAnimals = [
  ...
  */
  ...
  function lowPopulationAnimals(){
    const filtered = zooAnimals.filter((count) => {
  ...
  */
  ...
  function USApop(){
    const totalPopulation = zooAnimals.reduce((currentTotal, value) => {
      return value.population + currentTotal
    }, 0)
